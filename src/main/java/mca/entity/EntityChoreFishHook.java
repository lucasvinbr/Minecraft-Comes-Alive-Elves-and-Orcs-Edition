...
public class EntityChoreFishHook extends Entity {
	...
	public EntityChoreFishHook(World worldIn, double x, double y, double z, EntityVillagerMCA anglerIn) {
		...
		angler.getBehavior(ActionFish.class).setHookEntity(this);
	}
	...
	public EntityChoreFishHook(World worldIn, EntityVillagerMCA anglerIn) {
		...
		angler.getBehavior(ActionFish.class).setHookEntity(this);
		this.setSize(0.25F, 0.25F);
		this.setLocationAndAngles(anglerIn.posX, anglerIn.posY + (double) anglerIn.getEyeHeight(), anglerIn.posZ,
				anglerIn.rotationYaw, anglerIn.rotationPitch);
	...
	public void handleHookCasting(double p_146035_1_, double p_146035_3_, double p_146035_5_, float p_146035_7_,
			float p_146035_8_) {
		...
		this.prevRotationYaw = this.rotationYaw = (float) (MathHelper.atan2(p_146035_1_, p_146035_5_) * 180.0D
				/ Math.PI);
		this.prevRotationPitch = this.rotationPitch = (float) (MathHelper.atan2(p_146035_3_, (double) f1) * 180.0D
				/ Math.PI);
		...
		if (this.fishPosRotationIncrements > 0) {
			...
			if (!this.world.isRemote && this.angler != null) {
				...
				if (this.angler.isDead || !this.angler.isEntityAlive() || itemstack == null
					...
					this.setDead();
					angler.getBehavior(ActionFish.class).setHookEntity(null);
					return;
				}
			...
			if (!this.inGround) {
				...
				for (this.rotationPitch = (float) (MathHelper.atan2(this.motionY, (double) f5) * 180.0D
				...
				if (!this.world.isRemote && d10 > 0.0D) {
					...
					if (this.ticksCatchable > 0) {
						...
						if (rand.nextInt(10) == 0) {
							if (!world.isRemote) {
								try {
									angler.getBehavior(ActionFish.class).doFishCatchAttempt();
									angler.getBehavior(ActionFish.class).setHookEntity(null);
									setDead();
									ticksCatchable = 0;
									angler.swingItem();
									...
									LogManager.getLogger(this.getClass()).warn(e.getLocalizedMessage(), e);
								}
							}
						}
					}
