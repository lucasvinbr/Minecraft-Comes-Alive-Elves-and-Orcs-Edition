	...
	public void processOnGameThread(PacketEditVillager message, MessageContext context) {
		...
		if (player != null && villager != null) {
			...
			case SET_NAME:
				...
				break;
			case SWAP_GENDER:
				villager.attributes.setGender(
						villager.attributes.getGender() == EnumGender.MALE ? EnumGender.FEMALE : EnumGender.MALE);
				villager.attributes.assignRandomSkin();
				break;
			case TEXTURE_DOWN:
				villager.attributes.setHeadTexture(
						villager.attributes.getHeadTexture().replaceAll("\\d+", String.valueOf(textures.previous())));
				villager.attributes.setClothesTexture(villager.attributes.getHeadTexture());
				break;
			case TEXTURE_UP:
				villager.attributes.setHeadTexture(
						villager.attributes.getHeadTexture().replaceAll("\\d+", String.valueOf(textures.next())));
				villager.attributes.setClothesTexture(villager.attributes.getHeadTexture());
				break;
