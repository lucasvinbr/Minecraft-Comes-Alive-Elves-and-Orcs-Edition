...
public class PacketGift extends AbstractPacket<PacketGift> {
	...
	private boolean handleWeddingRing(EntityPlayer player, EntityVillagerMCA human) {
		...
		if (human.attributes.getIsChild() || !human.attributes.allowsIntimateInteractions(player)) {
			human.say("interaction.give.invalid", player);
				...
				&& human.attributes.getMarriageState() == EnumMarriageState.ENGAGED) {
			...
			TutorialManager.sendMessageToPlayer(player, "You are now married. You can have",
					"children by using the 'Procreate' button.");

			List<EntityVillagerMCA> nearbyVillagers = RadixLogic.getEntitiesWithinDistance(EntityVillagerMCA.class,
					player, 30);
			...
			human.say("interaction.marry.fail.marriedtoother", player);
				...
				&& human.attributes.getSpouseUUID() != data.getUUID()) {
			...
			TutorialManager.sendMessageToPlayer(player, "You must have 5 golden hearts with",
					"a villager before marrying them.");
		}
	...
	private boolean handleMatchmakersRing(EntityPlayer player, EntityVillagerMCA human, ItemStack stack) {
		EntityVillagerMCA partner = RadixLogic.getClosestEntity(Point3D.fromEntityPosition(human), human.world, 5,
				EntityVillagerMCA.class, human);

		if (human.attributes.getIsChild()) {
			human.say("interaction.give.invalid", player);
		}

		else if (human.attributes.getMarriageState() == EnumMarriageState.MARRIED_TO_PLAYER
				|| human.attributes.getMarriageState() == EnumMarriageState.MARRIED_TO_VILLAGER) {
			...
			TutorialManager.sendMessageToPlayer(player, "You must have two matchmaker's rings",
					"in a stack to arrange a marriage.");
		}
			...
			boolean partnerIsValid = partner != null
					...
					&& (partner.attributes.getFatherUUID() == Constants.EMPTY_UUID
							|| partner.attributes.getFatherUUID() != human.attributes.getFatherUUID())
					&& (partner.attributes.getMotherUUID() == Constants.EMPTY_UUID
							|| partner.attributes.getMotherUUID() != human.attributes.getMotherUUID());

			if (partner == null) {
				human.say("interaction.matchmaker.fail.novillagers", player);
				TutorialManager.sendMessageToPlayer(player, "To arrange a marriage, have two rings in a stack and ",
						"make sure another marriable villager is nearby.");
			}
				...
				TutorialManager.sendMessageToPlayer(player,
						"A married villager, relative, or child was too close to this",
						"villager. Move this villager away from anyone not marriable.");
			}
				...
				for (Object obj : human.world.playerEntities) {
					...
					if (human.attributes.isPlayerAParent(onlinePlayer)
							|| partner.attributes.isPlayerAParent(onlinePlayer)) {
						// onlinePlayer.addStat(AchievementsMCA.childMarried);
					}
				}

				TutorialManager.sendMessageToPlayer(player, "These villagers are now married.",
						"They will have children in the near future.");
				return true;
			}
		}
	...
	private boolean handleEngagementRing(EntityPlayer player, EntityVillagerMCA human) {
		...
		if (human.attributes.getIsChild() || !human.attributes.allowsIntimateInteractions(player)) {
			...
			human.getBehavior(ActionUpdateMood.class).modifyMoodLevel(3.0F);
			Utilities.spawnParticlesAroundEntityS(EnumParticleTypes.HEART, human, 16);
			TutorialManager.sendMessageToPlayer(player, "You are now engaged. Now gift a wedding ring",
					"to get gifts from other villagers.");
			return true;
		}
	...
	private boolean handleStandardGift(EntityPlayer player, EntityVillagerMCA human, int slot1, ItemStack stack) {
		...
		final Object queryObject = stack.getItem() instanceof ItemBlock ? Block.getBlockFromItem(stack.getItem())
				: stack.getItem();
		int giftValue = RegistryMCA.getGiftMap().containsKey(queryObject) ? RegistryMCA.getGiftMap().get(queryObject)
				: -5;
		...
		final int heartsModify = RadixMath.clamp(giftValue - (memory.getInteractionFatigue() * 4), -5,
				Integer.MAX_VALUE);
		final String giftType = heartsModify <= 0 ? "bad"
				: heartsModify <= 5 ? "good" : heartsModify <= 10 ? "better" : "best";
		...
		if (giftValue > 0 && heartsModify <= 0) {
			TutorialManager.sendMessageToPlayer(player, "You have interacted with this villager too much.",
					"Wait before interacting again.");
		}

		else if (giftValue <= 0) {
			TutorialManager.sendMessageToPlayer(player, "This villager did not like your gift.",
					"Try a different item next time.");
		}
	...
	public void processOnGameThread(PacketGift packet, MessageContext context) {
		...
		if (player != null && human != null) {
			...
			if (item == ItemsMCA.WEDDING_RING || item == ItemsMCA.WEDDING_RING_RG) {
			...
			else if (item == ItemsMCA.BOOK_ROSE_GOLD && human.getName().equals("William")
					&& human.attributes.getProfessionEnum() == EnumProfession.Miner) {
				removeItem = true;
				human.sayRaw("Oh, thank you for returning this to me! My company secrets could have been lost forever!",
						player);
			}

			else if (human.attributes.getIsInfected() && human.getActivePotionEffect(MobEffects.WEAKNESS) != null
			...
			else if (item == Items.GOLDEN_APPLE && human.attributes.getIsChild()
					&& human.attributes.isPlayerAParent(player)) {
				removeItem = true;
				removeCount = 1;
			...
			else if ((item == ItemsMCA.BABY_BOY || item == ItemsMCA.BABY_GIRL)
					&& human.attributes.getPlayerSpouseInstance() == player) {
				removeItem = true;
				removeCount = 1;
				...
				if (human.attributes.isMarriedToAVillager() && human.attributes.getVillagerSpouseInstance() != null
						&& RadixMath.getDistanceToEntity(human, human.attributes.getVillagerSpouseInstance()) <= 8.5D) {
					...
					if (human.attributes.getBabyState() == EnumBabyState.NONE
							&& spouse.attributes.getBabyState() == EnumBabyState.NONE) {
						human.say("gift.cake" + RadixMath.getNumberInRange(1, 3), player);

						final EntityVillagerMCA progressor = !human.attributes.getIsMale() ? human
								: !spouse.attributes.getIsMale() ? spouse : human;
						human.getBehavior(ActionStoryProgression.class)
								.setProgressionStep(EnumProgressionStep.HAD_BABY);
						spouse.getBehavior(ActionStoryProgression.class)
								.setProgressionStep(EnumProgressionStep.HAD_BABY);
				...
				else if (human.attributes.isMarriedToAVillager()
						&& human.attributes.getVillagerSpouseInstance() == null) {
				...
				human.attributes.setClothesTexture(
						human.attributes.getProfessionSkinGroup().getRandomMaleSkin(human.attributes.getRaceEnum()));
				removeItem = true;
				removeCount = 1;
			}
				...
				inventory.setInventorySlotContents(inventorySlot,
