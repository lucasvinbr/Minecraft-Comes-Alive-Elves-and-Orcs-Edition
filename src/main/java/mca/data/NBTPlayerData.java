...
public final class NBTPlayerData implements Serializable {
	...
	public void setGender(EnumGender gender) {
		...
		broadcastValueChange(FieldUpdateObj.get(FieldID.GENDER, TypeID.INT, gender.getId()));
	}

	public EnumRace getRace() {
		return EnumRace.getRaceById(race);
	}

	public EnumRace getSpouseRace() {
		return EnumRace.getRaceById(spouseRace);
	}


	public void setSpouseGender(EnumGender gender) {
		this.spouseGender = gender.getId();
		broadcastValueChange(FieldUpdateObj.get(FieldID.SPOUSE_GENDER, TypeID.INT, gender.getId()));
	}
	
	public void setRace(EnumRace race) {
		this.race = race.getId();
		broadcastValueChange(FieldUpdateObj.get(FieldID.GENDER, TypeID.INT, race.getId()));
	}

	public void setSpouseRace(EnumRace race) {
		this.spouseRace = race.getId();
		broadcastValueChange(FieldUpdateObj.get(FieldID.SPOUSE_GENDER, TypeID.INT, race.getId()));
	}
			...
			if (ownerUUID != null) //Make sure it was found.
			{
				...
				if (player != null) {
					MCA.getPacketHandler()
							.sendPacketToPlayer(new PacketPlayerDataC(fieldUpdate), (EntityPlayerMP) player);
				} else {
					LogManager.getLogger(NBTPlayerData.class).error("Error getting player instance by UUID");
				}
			} else {
				LogManager.getLogger(NBTPlayerData.class).error("Error looking up player by UUID");
			}
		}
	}
	...
	public void setByFieldUpdateObj(FieldUpdateObj obj) {
		...
		switch (obj.fieldId) {
			case UUID:
				setUUID((UUID) obj.value);
				break;
			case SPOUSE_UUID:
				setSpouseUUID((UUID) obj.value);
				break;
			case GENDER:
				setGender(EnumGender.byId((Integer) obj.value));
				break;
			case SPOUSE_GENDER:
				setSpouseGender(EnumGender.byId((Integer) obj.value));
				break;
			case OWNS_BABY:
				setOwnsBaby((Boolean) obj.value);
				break;
			case MARRIAGE_STATE:
				setMarriageState(EnumMarriageState.byId((Integer) obj.value));
				break;
			case IS_NOBILITY:
				setIsNobility((Boolean) obj.value);
				break;
			case MCA_NAME:
				setMcaName((String) obj.value);
				break;
			case HAS_CHOSEN_DESTINY:
				setHasChosenDestiny((Boolean) obj.value);
				break;
			case GENDER_PREFERENCE:
				setGenderPreference(EnumGender.byId((Integer) obj.value));
				break;
			case IS_SUPER_USER:
				setIsSuperUser((Boolean) obj.value);
				break;
			case SPOUSE_NAME:
				setSpouseName((String) obj.value);
				break;
			case HAPPINESS_THRESHOLD_MET:
				setHappinessThresholdMet((Boolean) obj.value);
				break;
		}
