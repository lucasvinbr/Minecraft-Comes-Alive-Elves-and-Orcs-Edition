...
public class GuiPlayerMenu extends GuiScreen {
	...
	public GuiPlayerMenu(EntityPlayer player, EntityPlayer target, boolean targetIsMarried, boolean targetIsEngaged,
			boolean isMarriedToInitiator, String targetSpouseName) {
		super();
		this.player = player;
	...
	public void drawScreen(int i, int j, float f) {
		...
		GL11.glPushMatrix();
		{
			...
			RadixRender.drawTexturedRectangle(new ResourceLocation("mca:textures/gui.png"), 5, 30, marriageIconU, 0, 16,
					16);
		}
		GL11.glPopMatrix();

		if (displayMarriageInfo) {
			String phraseId = isMarriedToInitiator ? "gui.info.family.marriedtoplayer"
					: targetIsMarried ? "gui.info.family.married"
							: targetIsEngaged ? "gui.info.family.engaged" : "gui.info.family.notmarried";
	...
	protected void actionPerformed(GuiButton button) {
		...
		if (interaction != null) {
			MCA.getPacketHandler()
					.sendPacketToServer(new PacketInteractWithPlayerS(interaction.getId(), playerTarget.getEntityId()));
		}
	...
	private void drawMainButtonMenu() {
		...
		if (isMarriedToInitiator) {
			buttonList.add(new GuiButton(EnumInteraction.DIVORCE.getId(), width / 2 + xLoc, height / 2 - yLoc, 65, 20,
					MCA.getLocalizer().getString("gui.button.divorce")));
			yLoc -= yInt;
			buttonList.add(new GuiButton(EnumInteraction.HAVEBABY.getId(), width / 2 + xLoc, height / 2 - yLoc, 65, 20,
					MCA.getLocalizer().getString("gui.button.havebaby")));
			yLoc -= yInt;
		}

		else {
			buttonList.add(new GuiButton(EnumInteraction.ASKTOMARRY.getId(), width / 2 + xLoc - 10, height / 2 - yLoc,
					75, 20, MCA.getLocalizer().getString("gui.button.asktomarry")));
			yLoc -= yInt;
		}
	}
