...
public class GuiSetup extends GuiScreen {
	...
	public void drawScreen(int sizeX, int sizeY, float offset) {
		...
		if (page == 1) {
			...
			drawCenteredString(fontRenderer, "What is your race?", width / 2, 120, 0xffffff);
		}
		else if (page == 4 && nameTextField != null) {
			...
			drawCenteredString(fontRenderer, "WARNING: This destiny can potentially be destructive to your world.",
					width / 2, 70, 0xffffff);
			drawCenteredString(fontRenderer,
					"This option works best on flat land with no other structures nearby. Continue?", width / 2, 85,
					0xffffff);
		}
	...
	private void cleanUpOnClose() {
		try {
			Map<Point3D, BlockObj> destinySchematic = RadixSchematics
					.readSchematic("/assets/mca/schematic/destiny-test.schematic");
			...
			for (Map.Entry<Point3D, BlockObj> entry : destinySchematic.entrySet()) {
				...
				if (y > (int) player.posY - 2) {
					RadixBlocks.setBlock(player.world, MCA.destinyCenterPoint.iX() + entry.getKey().iX(), y,
	...
	protected void actionPerformed(GuiButton button) {
		...
		switch (button.id) {
		case 1:
			data.setGender(EnumGender.MALE);
			break;
		case 2:
			data.setGender(EnumGender.FEMALE);
			break;
		case 3:
			data.setGenderPreference(EnumGender.MALE);
			break;
		case 4:
			data.setGenderPreference(EnumGender.UNASSIGNED);
			break;
		case 5:
			data.setGenderPreference(EnumGender.FEMALE);
			break;
		case 6:
			...
			// Skip destiny choices if on single player and the option is disabled, or if
			// we're on
			// a dedicated server and the server option is disabled. Server option is
			// disabled by
		...
		case 7:
			destinyChoice = EnumDestinyChoice.FAMILY;
			page = 6;
			break;
		case 8:
			destinyChoice = EnumDestinyChoice.ALONE;
			page = 6;
			break;
		case 9:
			destinyChoice = EnumDestinyChoice.VILLAGE;
			page = 6;
			break;
		case 10: // No destiny.
		...
		case 13:
			...
			break;
		case 14: 
			data.setRace(EnumRace.Villager);
			page = 4;
			break;
		case 15:
			data.setRace(EnumRace.Orc);
			page = 4;
			break;
		case 16:
			data.setRace(EnumRace.Elf);
			page = 4;
			break;
		}
	}
	...
	private void drawControls() {
		...
		if (page == 1) {
			...
			buttonList.add(new GuiButton(14, width / 2 - 97, height / 2 + 10, 65, 20, Color.BLUE + "Humans"));
			buttonList.add(new GuiButton(15, width / 2 - 32, height / 2 + 10, 65, 20, Color.DARKGREEN + "Orcs"));
			buttonList.add(new GuiButton(16, width / 2 + 33, height / 2 + 10, 65, 20, Color.GOLD + "Elves"));
		}
		else if (page == 4) {
			...
			GuiButton doneButton = new GuiButton(6, width / 2 - 32, height / 2 + 30, 65, 20,
					MCA.getConfig().enableStructureSpawning ? "Continue" : "Done");
			...
			buttonList.add(new GuiButton(9, width / 2 - 46, height / 2, 95, 20, "I live in a village."));
			...
			buttonList.add(new GuiButton(12, width / 2 - 46, height / 2, 95, 20, "No"));
