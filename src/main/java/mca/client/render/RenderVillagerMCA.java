...
public class RenderVillagerMCA<T extends EntityVillagerMCA> extends RenderBiped<EntityVillagerMCA> {
	...
	protected void preRenderCallback(EntityVillagerMCA entityLivingBase, float partialTickTime) {
		...
		float
				scale =
				entity.attributes.getGender() == EnumGender.MALE ? Constants.SCALE_M_ADULT : Constants.SCALE_F_ADULT;

		if (entity.attributes.getIsChild()) {
			final boolean doGradualGrowth = MCA.getConfig().isAgingEnabled;
			final float
					growthFactor =
					(entity.attributes.getGender() == EnumGender.MALE ? 0.39F : 0.37F) /
							MCA.getConfig().childGrowUpTime * entity.attributes.getAge();
	...
	private void doRenderEntity(EntityVillagerMCA entity,
			double x,
			double y,
			double z,
			float entityYaw,
		...
		if (RadixMath.getDistanceToEntity(entity, player) <= 5.0F &&
				!entity.getBehavior(ActionSleep.class).getIsSleeping() &&
				!entity.isInteractionGuiOpen &&
				memory != null) {
			UVPoint
					uvp =
					memory.doDisplayFeedback() ?
					(memory.getLastInteractionSuccess() ? plus : minus) :
			...
			if (uvp != null) {
				GL11.glPushMatrix();
				{
					...
					RadixRender.drawTexturedRectangle(gui,
							(int) x,
							(int) y + 12,
							uvp.getU(),
							uvp.getV(),
	 ...
	 */
	private void doRenderEffects(EntityVillagerMCA entity,
			double x,
			double y,
			double z,
			float entityYaw,
		...
		//When in the interaction GUI, render the person's name and hearts value.
			...
			renderHearts(entity,
					...
					y + 0.1,
					z,
					entity.attributes.getPlayerMemory(Minecraft.getMinecraft().player).getHearts());
		}
	...
	private void renderHealth(EntityVillagerMCA human,
			double posX,
			double posY,
			double posZ,
			int currentHealth,
	...
	private boolean canRenderNameTag(EntityLivingBase entityRendering) {
		...
		final Vec3d
				entityLookVector =
				new Vec3d(entityRendering.posX - entityPlayer.posX,
						entityRendering.getEntityBoundingBox().minY - 3 + (double) entityRendering.height / 2.0F -
								entityPlayer.posY + entityPlayer.getEyeHeight(),
						entityRendering.posZ -
								entityPlayer.posZ).normalize(); //entityRendering.getLook(1.0F).normalize();
		...
		return !(Minecraft.getMinecraft().currentScreen instanceof GuiInteraction) &&
				...
				entityRendering != Minecraft.getMinecraft().player &&
				!entityRendering.isInvisibleToPlayer(Minecraft.getMinecraft().player) &&
				!entityRendering.isBeingRidden();
	}
