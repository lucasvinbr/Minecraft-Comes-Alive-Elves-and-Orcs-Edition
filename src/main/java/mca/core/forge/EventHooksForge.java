...
public class EventHooksForge {
	...
	public void onLootTableLoad(LootTableLoadEvent event) {
		if (event.getName().toString().equals("minecraft:chests/simple_dungeon")
			...
			main.addEntry(new LootEntryItem(ItemsMCA.BOOK_DEATH, 20, 0, new LootFunction[0], new LootCondition[0],
					"mca:book_death"));
			main.addEntry(new LootEntryItem(ItemsMCA.BOOK_FAMILY, 20, 0, new LootFunction[0], new LootCondition[0],
					"mca:book_family"));
			main.addEntry(new LootEntryItem(ItemsMCA.BOOK_INFECTION, 20, 0, new LootFunction[0], new LootCondition[0],
					"mca:book_infection"));
			main.addEntry(new LootEntryItem(ItemsMCA.BOOK_ROMANCE, 20, 0, new LootFunction[0], new LootCondition[0],
					"mca:book_romance"));
			main.addEntry(new LootEntryItem(ItemsMCA.BOOK_ROSE_GOLD, 20, 0, new LootFunction[0], new LootCondition[0],
					"mca:book_rose_gold"));
		}
	}
	...
	public void entityJoinedWorldEventHandler(EntityJoinWorldEvent event) {
		if (!event.getWorld().isRemote
				&& MCA.getConfig().canSpawnInDimension(event.getWorld().provider.getDimension())) {
			...
			if (event.getEntity().getClass() == EntityVillager.class && MCA.getConfig().overwriteOriginalVillagers) {
				...
				// Check for a zombie being turned into a villager. Don't overwrite with
				// families in this case.
				List<EntityZombieVillager> zombiesAroundMe = RadixLogic
						.getEntitiesWithinDistance(EntityZombieVillager.class, event.getEntity(), 3);
				...
				if (villager.getProfession() >= 0 && villager.getProfession() <= 4) {
					...
					// This will prevent ConcurrentModification exceptions when overwriting
					// villagers.
	...
	public void entityInteractEventHandler(EntityInteract event) {
		if (!(event.getEntityPlayer() instanceof FakePlayer)) {
			...
			else if (event.getTarget() instanceof EntityPlayerMP && !event.getEntityPlayer().world.isRemote
					&& !event.getEntityPlayer().getName().contains("[CoFH]")) {
				MCA.getPacketHandler().sendPacketToPlayer(
						new PacketInteractWithPlayerC(event.getEntityPlayer(), (EntityPlayer) event.getTarget()),
	...
	public void onLivingAttack(LivingAttackEvent event) {
		// Not interested in fake players due to the inability to generate player data
		// for them.
		...
		if (event.getEntityLiving() instanceof EntityPlayer
			...
			List<EntityVillagerMCA> entityList = RadixLogic.getEntitiesWithinDistance(EntityVillagerMCA.class,
					event.getEntityLiving(), 15);
		...
		// Handle warrior triggers on player dealing damage.
			...
			List<EntityVillagerMCA> entityList = RadixLogic.getEntitiesWithinDistance(EntityVillagerMCA.class,
					event.getSource().getTrueSource(), 15);

			for (Entity entity : entityList) {
				EntityVillagerMCA human = (EntityVillagerMCA) entity;
				ActionCombat combat = human.getBehavior(ActionCombat.class);
				PlayerMemory memory = human.attributes
						.getPlayerMemory((EntityPlayer) event.getSource().getTrueSource());
		...
		if (MCA.getConfig().enableInfection) {
			if (event.getSource() != null && event.getSource().getTrueSource() instanceof EntityZombie) {
				...
				if (event.getEntityLiving() instanceof EntityPlayer && flag) {
					...
					for (ItemStack stack : player.inventory.mainInventory) {
						if (stack != null && stack.getItem() instanceof ItemBaby) {
							...
							player.sendMessage(
									new TextComponentString(Color.RED + stack.getTagCompound().getString("name")
											+ " has been " + Color.GREEN + Format.BOLD + "infected" + Color.RED + "!"));
							player.world.playSound(null, new BlockPos(zombie.posX, zombie.posY, zombie.posZ),
									SoundEvents.ENTITY_WITHER_AMBIENT, SoundCategory.HOSTILE, 0.5F, 1.0F);
				...
				else if (event.getEntityLiving() instanceof EntityVillagerMCA
						&& ((EntityVillagerMCA) event.getEntityLiving()).attributes.getIsInfected()) {
	...
	public void onLivingSetTarget(LivingSetAttackTargetEvent event) {
		// Mobs shouldn't attack infected villagers. Account for this when they attempt
		// to set their target.
		...
		if (placedBlock == Blocks.FIRE && RadixBlocks.getBlock(event.getWorld(), x, y - 1, z) == Blocks.EMERALD_BLOCK) {
			...
			for (int i = 0; i < 4; i++) {
				...
				for (int j = -1; j < 2; j++) // -1 since the fire is on top of the emerald.
				{
					...
					// If we made it up to 1 without breaking, make sure the block is fire so that
					// it's a lit totem.
			...
			if (totemsFound >= 3 && !event.getWorld().isDaytime()) {
				...
				NetworkRegistry.TargetPoint summonTarget = new NetworkRegistry.TargetPoint(
						event.getWorld().provider.getDimension(), summonPoint.iX(), summonPoint.iY() + 5,
						summonPoint.iZ(), 32);

				EventHooksFML.setReaperSummonPoint(event.getWorld(), new Point3D(x + 1.0D, y + 5, z + 1.0D));

				// FIXME
				// MCA.getPacketHandler().sendPacketToAllAround(new
				// PacketPlaySoundOnPlayer("portal.portal"), summonTarget);
