		...
		if (!worldIn.isRemote) {
			...
			if (tile instanceof TileMemorial) {
				...
				if (!playerData.getIsSuperUser() && !memorial.getOwnerUUID().equals(playerData.getUUID())) {
					playerIn.sendMessage(new TextComponentString(Color.RED +
							"You cannot revive " +
							data.getName() +
							" because they are not related to you."));
					return EnumActionResult.FAIL;
				}

				//For rings, they belonged to a spouse. Check for remarriage and forbid.
				if (memorial.getType() == EnumMemorialType.BROKEN_RING &&
						(playerData.getMarriageState() != EnumMarriageState.NOT_MARRIED)) {
					playerIn.sendMessage(new TextComponentString(Color.RED +
							"You cannot revive " +
							data.getName() +
