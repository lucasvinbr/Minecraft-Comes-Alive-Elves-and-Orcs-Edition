...
public class ActionFish extends AbstractToggleAction {
	...
	public void onUpdateServer() {
		...
		try {
			...
			if (!canFishingBegin()) {
				actor.getNavigator()
						.setPath(actor.getNavigator()
										.getPathToXYZ(waterCoordinatesX, waterCoordinatesY, waterCoordinatesZ),
		...
		if (RadixLogic.getBooleanWithProbability(catchChance)) {
			try {
				...
				if (actor.attributes.getInventory().containsCountOf(Items.FISH, 64)) {
					actor.say(MCA.getLocalizer().getString("notify.child.chore.finished.fishing"),
							getAssigningPlayer());
					reset();
					return;
				}
