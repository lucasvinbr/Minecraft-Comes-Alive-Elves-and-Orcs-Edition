...
public class ActionCombat extends AbstractAction {
	private static final DataParameter<Integer> ATTACK_METHOD_ID = EntityDataManager
			.<Integer>createKey(EntityVillagerMCA.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> ATTACK_TRIGGER_ID = EntityDataManager
			.<Integer>createKey(EntityVillagerMCA.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> ATTACK_TARGET_ID = EntityDataManager
			.<Integer>createKey(EntityVillagerMCA.class, DataSerializers.VARINT);
	...
	public void onUpdateServer() {
		...
		// If we have a target, proceed to attack.
			...
			// Ranged attacks
					...
					|| (getMethodBehavior() == EnumCombatBehaviors.METHOD_MELEE_AND_RANGED
							&& distanceToTarget >= 5.0F)) {
				actor.getLookHelper().setLookPosition(attackTarget.posX,
						attackTarget.posY + (double) attackTarget.getEyeHeight(), attackTarget.posZ, 10.0F,
				...
				if (rangedAttackTime <= 0) {
					...
					double dY = attackTarget.getEntityBoundingBox().minY + (double) (attackTarget.height / 3.0F)
							- arrow.posY;
					double dZ = attackTarget.posZ - actor.posZ;
					double d3 = (double) MathHelper.sqrt(dX * dX + dZ * dZ);

					arrow.setThrowableHeading(dX, dY + d3 * 0.20000000298023224D, dZ, 1.6F,
							(float) (14 - actor.world.getDifficulty().getDifficultyId() * 4));
					arrow.setDamage((double) (5.0F) + actor.getRNG().nextGaussian() * 0.25D
							+ (double) ((float) actor.world.getDifficulty().getDifficultyId() * 0.11F));
	...
	private void findAttackTarget() {
		List<EntityLivingBase> entitiesAroundMe = RadixLogic.getEntitiesWithinDistance(EntityLivingBase.class, actor,
				10);
	...
	public boolean isEntityValidToAttack(EntityLivingBase entity) {
		if (entity instanceof EntityMob && (getTargetBehavior() == EnumCombatBehaviors.TARGET_HOSTILE_MOBS
	...
	protected void registerDataParameters() {
		...
		actor.getDataManager().register(ATTACK_TRIGGER_ID,
