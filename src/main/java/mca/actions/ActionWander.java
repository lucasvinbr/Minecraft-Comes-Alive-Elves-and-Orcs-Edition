...
public class ActionWander extends AbstractAction {
	...
	public void onUpdateServer() {
		// Prevent the workday while we're down for sleep or doing something with the
		// player.
	...
	private void getLookTarget(boolean playerOnly) {
		...
		final List<Entity> entitiesAroundMe = actor.world.getEntitiesWithinAABB(entityClass,
				new AxisAlignedBB(actor.posX - maxDistanceAway, actor.posY - maxDistanceAway,
						actor.posZ - maxDistanceAway, actor.posX + maxDistanceAway, actor.posY + maxDistanceAway,
						actor.posZ + maxDistanceAway));
