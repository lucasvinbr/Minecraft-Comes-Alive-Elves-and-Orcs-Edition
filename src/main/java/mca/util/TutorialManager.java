...
public final class TutorialManager {
		...
		if (MCA.getConfig().inTutorialMode) {
			if (firstMessageShown && !turnOffMessageShown) {
				...
				if (timeSinceFirstMessage >= 500) {
					setTutorialMessage(new TutorialMessage("You can turn off these messages at the Main Menu.",
							"Select Mods->Minecraft Comes Alive->Config->Tutorial mode->false."));
	...
	public static void sendMessageToPlayer(EntityPlayer player, String line1, String line2) {
		MCA.getPacketHandler().sendPacketToPlayer(new PacketSetTutorialMessage(new TutorialMessage(line1, line2)),
