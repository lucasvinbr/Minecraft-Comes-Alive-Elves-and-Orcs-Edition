...
public class CommandMCA extends CommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender commandSender, String[] input) throws CommandException {
		org.apache.logging.log4j.LogManager.getLogger(this.getClass().getName()).log(
				org.apache.logging.log4j.Level.DEBUG,
				java.text.MessageFormat.format(
						"Executing MCA Command: MinecraftServer: {0}\r\nICommandSender: {1}\r\nInput:{2}",
						server, commandSender, input));
		FMLLog.log(org.apache.logging.log4j.Level.DEBUG,
				java.text.MessageFormat.format(
						"Executing MCA Command: MinecraftServer: {0}\r\nICommandSender: {1}\r\nInput:{2}", server,
						commandSender, input));
		...
		try {
			...
			if (subcommand.equalsIgnoreCase("help")) {
				...
				for (Object obj : player.world.loadedEntityList) {
					if (obj instanceof EntityVillagerMCA) {
						...
						if (human.attributes.getIsChild()) {
							...
							float newHeight = 0.69F
									+ (human.attributes.getAge() * (1.8F - 0.69F) / MCA.getConfig().childGrowUpTime);
				...
				if (targetPlayer != null) {
					EntityVillagerMCA spouse = (EntityVillagerMCA) MCA.getEntityByUUID(targetPlayer.world,
							targetPlayerData.getSpouseUUID());
				...
				if (targetPlayer != null) {
					...
					sendMessage(commandSender,
							Color.GOLD + playerName + "'s hearts for all loaded villagers were reset.");
			...
			else if (subcommand.equalsIgnoreCase("tpn")) // Toggle player nobility
			{
				...
				IngameTester.stressTest(player);
			}
	...
	private void displayHelp(ICommandSender commandSender) {
		sendMessage(commandSender, Color.DARKRED + "--- " + Color.GOLD + "DEBUG COMMANDS" + Color.DARKRED + " ---",
				true);

		sendMessage(commandSender, Color.WHITE + " /mca ffh " + Color.GOLD + " - Force all hearts on all villagers.",
				true);
		...
		sendMessage(commandSender, Color.WHITE + " /mca fsp " + Color.GOLD + " - Force story progression to continue.",
				true);
		sendMessage(commandSender,
				Color.WHITE + " /mca cli " + Color.GOLD + " - Clear interaction flag on all villagers.", true);
		sendMessage(commandSender, Color.WHITE + " /mca clv " + Color.GOLD + " - Clear all loaded villagers. "
				+ Color.RED + "(IRREVERSABLE)", true);
		...
		sendMessage(commandSender, Color.WHITE + " /mca kgr " + Color.GOLD + " - Kill all Grim Reapers in the world.",
				true);
		sendMessage(commandSender, Color.WHITE + " /mca dpd " + Color.GOLD + " - Dump player data for <username>.",
				true);
		sendMessage(commandSender,
				Color.WHITE + " /mca cpd " + Color.GOLD + " - Convert old player data to the new format.", true);

		sendMessage(commandSender, Color.DARKRED + "--- " + Color.GOLD + "OP COMMANDS" + Color.DARKRED + " ---", true);
		sendMessage(commandSender,
				Color.WHITE + " /mca rm <username> " + Color.GOLD + " - Reset <username>'s marriage.", true);
		sendMessage(commandSender, Color.WHITE + " /mca rb <username> " + Color.GOLD + " - Reset <username>'s baby.",
				true);
		sendMessage(commandSender, Color.WHITE + " /mca rh <username> " + Color.GOLD + " - Reset <username>'s hearts.",
				true);
		sendMessage(commandSender,
				Color.WHITE + " /mca rr <username> " + Color.GOLD + " - Completely reset <username>.", true);
		sendMessage(commandSender, Color.WHITE + " /mca revive <uuid> " + Color.GOLD + " - Revive a dead villager.",
				true);
		sendMessage(commandSender,
				Color.WHITE + " /mca sudo <username> " + Color.GOLD + " - Toggle <username> as a superuser.", true);
		sendMessage(commandSender,
				Color.WHITE + " /mca sudo ? <username> " + Color.GOLD + " - Get if <username> is a superuser.", true);

		sendMessage(commandSender, Color.DARKRED + "--- " + Color.GOLD + "GLOBAL COMMANDS" + Color.DARKRED + " ---",
				true);
