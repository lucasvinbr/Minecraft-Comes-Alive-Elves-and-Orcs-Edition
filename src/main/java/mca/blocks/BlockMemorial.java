	...
	public void breakBlock(World world, BlockPos pos, IBlockState meta) {
		if (!world.isRemote) {
			...
			if (memorial != null) {
				...
				case BROKEN_RING:
					memorialItem = ItemsMCA.BROKEN_RING;
					break;
				case DOLL:
					memorialItem = ItemsMCA.CHILDS_DOLL;
					break;
				case TRAIN:
					memorialItem = ItemsMCA.TOY_TRAIN;
					break;
				}

				if (memorial.getRevivalTicks() == 0) {
					// Will be 1 when removed from a villager revival.
